...
17. train과 test를 함께!
train로직으로 모두 끝내어 pkl파일을 생성하면 이것으로 test를 바로 하고 끝나는 로직을 추가 및 -Test로 뒤에 이름을 붙임
...
>>초기에 3번일 동안을 했는데 너무 빨리 줄어 10으로 수정함
...
20. BatchNorm을 대신할 LayerNorm, GroupNorm 추가
21. 모델 실행시 Changes from the previous logic으로 이전과 바뀐 점 적음
22. 오버피팅과 관련해 stop하는 로직 추가
23. 최종 학습vs Val 기준 Best
마지막 상태(Final)와 최고 성적 상태(BastVal)를 대조하는 것은 모델의 과적합 여부와 학습 완성도를 판단하는데 좋음
24. floor 관련 로직 수정
체크포인트로 다시 시작시에 floor에 몇번 도달했는지 알아야함 현재 epoch_batch_losses를 체크 포인트 로직에 추가하지 않기에 이를 추가
또한 10몇번 도달했는지를 10이라 하드코딩했는데 이를 변수화함
lr전략1에서 floor로직의 경우 1에폭 안에서 현재 코드 기준 200번의 iter_per_epoch를 도는데 lr이 floor인 상태에서 2중for안에 cnt가 있으면 5번의 배치만에 5를 채우고 학습이 종료되니 모델이 최소 lr에서 epoch을 마무리해야 좋음>기존은 너무 성급한 종료
->이에 따라 에폭이 다 끝난 시점에서 lr이 floor인가를 체크
=>lr전략2에서만 floor로직 사용
25. val_loss는 필요한가?
gradient을 통해 loss,grad를 넘겨받는 로직은 많은 학습 시간을 축소시킴 근데 get_loss_in_batches 함수에서도 loss를 구하는데 이는 오직 val에 대한 loss를 구하기 위함임
그런데 valLoss에 대한 그래프를 그리지 않고 Early Stopping이나 Best Params 저장 기준은 valAcc니까 현재 로직 기준에선 없어도 됨
26. val_loss를 써보자
val_loss는 모델의 검증성을 알려줄 것임 val_loss가 잘 내려가다가 갑자기 2-4배씩 뛰면 사실 오버피팅이 시작되는 시점을 알려주는 것이 아닌가?>사용해보자
성능 개선)val의 손실과 정확도를 따로 구하면 Forward를 두 번 수행한다(accuracy와 loss함수는 같은 x,t로 self.predict(x)을 통해 동일한 Forward 연산을 하기 때문)->evaluate 함수 하나로 통합하여 한 번의 연산해 acc,loss를 구하도록 추가
"""
...
def get_loss_in_batches(network, x, t, batch_size=240):
...
def evaluate_model(network, x, t, batch_size=240):
    # val을 위한 로직
    """한 번의 Forward로 Loss와 Acc를 동시에 계산 (연산량 50% 절감)"""
    total_loss = 0
    total_acc = 0
    num_data = x.shape[0]
    ...
    for i in range(0, num_data, batch_size):
        ...
        # 1. 예측값(Y)을 구함 (이 과정에서 내부적으로 loss 계산을 위한 softmax까지 진행됨)
        # 2. loss와 accuracy를 한 번의 forward 흐름에서 추출
        ...
        total_acc += np.sum(y == t_batch)
        
    return total_loss / num_data, total_acc / num_data
## 시간 측정
...
best_val_loss,best_val_train_acc,best_val_epoch=0,0,0
